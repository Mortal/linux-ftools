...
char STR_FORMAT[] =  "%18s %18s %18s %18s %18s %18s %-70s\n";
//char STR_FORMAT[] =  "%-80s %18s %18s %18s %18s %18s %18s\n";
//char DATA_FORMAT[] = "%-80s %'18ld %'18d %'18ld %'18ld %'18ld %18.2f\n";
char DATA_FORMAT[] = "%18ld %18d %18ld %18ld %18ld %18.2f %-70s\n";
...
void fincore(char* path, 
	...
	if(is_dir(path)) {
		...
		char file_path[21024];
		
		while((ent=readdir(pDir))!=NULL) {
		
			file_path[0] = 0;
			
			if(path[0] != '/' || strlen(path) > 1) strcpy(file_path+strlen(file_path), path);
			
			strcpy(file_path+strlen(file_path), "/");
			
			strcpy(file_path+strlen(file_path), ent->d_name);
			
			//printf("travel : name[%s] file path[%s]\n",  ent->d_name, file_path);
			...
			fincore(file_path, result);
			
			this_page_size += result->cached_size;
		}
		//closedir(path);
		result->cached_size = this_page_size;
		return;
	}
...
void _show_headers() {
    ...
    printf( STR_FORMAT, "size", "total_pages", "min_cached page", "cached_pages", "cached_size", "cached_perc", "filename" );
    printf( STR_FORMAT, "----", "-----------", "---------------", "------------", "-----------", "-----------", "--------" );
